...
// real copy and the current version last, and the removal of all of it —
...
/** The route's source: the page's copy read at render, one lettered block per candidate, the current version last, inside the site's own layout. */
...
// \`agentic-cms demo new ${demo}\`, removed with the losing candidates by
...
import { kit } from "@/kit";
import { ${name} } from "${alias(file)}";
${candidates.map((c) => `import { ${c.name} } from "${alias(file)}${c.letter}";`).join("\n")}

export const metadata = { robots: { index: false } };

type Props = SectionProps<"${type}">;
const CANDIDATES: { letter: string; note: string; Candidate: (props: Props) => ReactNode }[] = [
${candidates.map((c) => `  { letter: "${c.letter}", note: "what differs, in one line", Candidate: ${c.name} },`).join("\n")}
];
...
export function writeDemo(root, { name, type, page, count = 2, component: override }) {
  ...
  if (!entry && !override) throw new Error(`${type}: no such section in ${REGISTRY} (${[...registry.keys()].join(", ")}); pass --component <file> for one outside the registry`);
  ...
  if (source === null) throw new Error(`${component.file}: no such file (the component of "${type}" in the registry)`);
  const pages = pagesWith(root, type);
  const slug = page ?? pages[0]?.slug;
  ...
  return { route: demoFile(name), path: `/${name}-demo`, section: type, page: slug, component: component.file, candidates, data: Boolean(component.data) };
}
...
/** Removes one demo: its folder, every component file (and its module.css) that the route alone imported, the stale route types. { removed, kept } */
