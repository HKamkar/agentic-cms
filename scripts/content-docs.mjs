#!/usr/bin/env node
// Writes the field tables of every collection into src/lib/content/README.md
// from the schemas themselves (their .describe() texts, the helpers' type
// names, ref() targets), so the documentation cannot drift from the contract:
//
//   node scripts/content-docs.mjs            rewrites the block between the markers
//   node scripts/content-docs.mjs --check    exits 1 when the block is out of date (pnpm build runs this)
//
...
const README = "src/lib/content/README.md";
...
const generated = [`${START}`, "<!-- generated by scripts/content-docs.mjs from the schemas in collections.ts; do not edit by hand -->", "", ...Object.values(collections).map(section), "", END].join("\n\n").replace(/\n\n\n+/g, "\n\n");
...
if (process.argv.includes("--check")) {
  if (next !== current) {
    console.error(`content-docs: ${README} is out of date — run node scripts/content-docs.mjs and commit the result`);
    process.exit(1);
  }
