...
- A demo route (`src/app/<name>-demo/`, written by `pnpm kit demo new`) and every losing candidate go before the branch merges (`pnpm kit demo clean`); the build's SEO audit fails on the route, which is the guard. A candidate's graphic is the meaning of the copy beside it; what is generic leaves the section for `ui/` in the same job.
