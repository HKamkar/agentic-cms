...
- A page is a file, `content/pages/<slug>.yaml`: its `seo` block (`path`, `title`, `description`, `ogImage`, `updated`, `breadcrumb`, `changeFrequency`, `priority`), its `jsonld` block (the copy of its structured data by page type) and its `sections`. `src/app/[[...slug]]/page.tsx` renders every page file with `pageMetadata()`, `pageBreadcrumb()` and `pageJsonLd()`. Never hand-write title, description, canonical, Open Graph or Twitter tags, and never add a `page.tsx` for a page made of existing sections. Posts: `postMetadata(post)` / `postBreadcrumb(post)`.
