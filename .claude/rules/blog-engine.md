...
- Frontmatter is a validated contract: `postSchema` in `src/lib/content/collections.ts`. New field = schema field (a helper + `.describe()`) + `toPost()` mapping + `PostMeta` + README + `_template.md`. Never read `data.<key>` ad hoc.
