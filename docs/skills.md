...
| `design-graphics` | a drawing the icon family's primitives cannot say: an icon of the site's own, a mark, an illustration, an animated logo or brand mark, a short 2D loop; "more graphical" | the artwork the site already has (its master, colours, proportions, sizes); motion ideas from the mark's parts and negative space; `pnpm kit lab new` / `serve` (the scene on the tokens, light and dark, at its sizes and enlarged, on a phone), `lab route` (the scenes on the site's own grounds, inside its chrome), the frames between and the loop boundary checked on the embedded `<img>`, `lab render` (the file a page ships, its still beside a loop), `icons add file:` (inline `Icon` data), the source kept, `lab clean`; `references/svg-motion.md` for the SVG details |
